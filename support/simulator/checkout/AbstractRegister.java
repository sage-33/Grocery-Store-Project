...
/**
 * <p>
 * A {@link AbstractRegister} is used to process {@link Shopper}s. A {@link AbstractRegister} has a
 * running cost associated with it. Each time a {@link Shopper} is processed, a
 * {@link AbstractRegister} will produce a {@link Transaction}. The running cost of a register is
 * associated with the skill level of the employee working at the register. That means, the faster
 * the employee is able to process a transaction, the higher the running cost will be.
 * </p>
 * <p>
...
public abstract class AbstractRegister {
    ...
    /**
     * Constructs an {@link Abstract} and connects it to {@link BigBrother}.
     */
    ...
    /**
     * Processes a {@link Shopper} producing a {@link Transaction}. In addition to processing the
     * {@link Shopper} the running cost is increased relative to the speed and difficulty of the
     * groceries purchased by the {@link Shopper}
     *
     * @param s
    ...
    public final Transaction processShopper(final Shopper s) {
        ...
        if (isBusy()) {
            throw new BigBrotherIsWatchingYouException("This register is already being used.");
        }
        ...
        runningCost += (difficulty * 5000) / (t.getTimeSteps() * t.getTimeSteps());
        transactionEndTime = BigBrother.getBigBrother().getTime() + t.getTimeSteps();
        transactions.add(t);
    ...
    /**
     * Every tick increases the cost of running this {@link AbstractRegister}. This is called by
     * {@link BigBrother} every time step.
     */
    ...
    /**
     * Returns {@code true} if this {@link AbstractRegister} is busy and {@code false} otherwise.
     *
     * @return {@code true} if this {@link AbstractRegister} is busy and {@code false} otherwise.
     */
    ...
    /**
     * Turns this {@link AbstractRegister} off. {@link BigBrother} will not allow you to turn this
     * {@link AbstractRegister} off if the register is busy.
     */
